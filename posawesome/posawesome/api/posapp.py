...
def get_opening_dialog_data():
    pos_porfiles=frappe.get_all("POS Profile User",filters={"user":frappe.session.user},fields=["parent"],pluck="parent")
    data = {}
    data["companies"] = frappe.get_all("Company", limit_page_length=0, order_by="name")
    data["pos_profiles_data"] = frappe.get_all(
        "POS Profile",
        filters={"name":["in",pos_porfiles],"disabled": 0},
