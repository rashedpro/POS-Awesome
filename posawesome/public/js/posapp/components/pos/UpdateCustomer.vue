            ...
            <v-row>
              ...
              <v-col cols="6">
                <v-autocomplete
                  ...
                  auto-select-first
                  color="primary"
                  :label="frappe._('Channel') + ' *'"
                  v-model="channels"
                  :items="custom_channels"
                  background-color="white"
                  :no-data-text="__('Channel not found')"
                  hide-details
                  required
                >
                </v-autocomplete>
...
export default {
  ...
  methods: {
    ...
    getCustomerChannels() {
      if (this.custom_channels.length > 0) return;
      const vm = this;
      frappe.db
        .get_list('Channels', {
          fields: ['name'],
        ...
        .then((data) => {
          if (data.length > 0) {
            data.forEach((el) => {
              vm.custom_channels.push(el.name);
            });
          }
        });
    },
    ...
    submit_dialog() {
      ...
      if (!this.channels) {
        evntBus.$emit('show_mesage', {
          text: __('Customer channels is required.'),
          color: 'error',
        });
        return;
      }
      if (!this.street) {
        evntBus.$emit('show_mesage', {
          text: __('Customer street is required.'),
          color: 'error',
        });
        return;
      }
